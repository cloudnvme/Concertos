    ...
    <table class="table table-condensed table-striped table-bordered table-hover">
      ...
      @foreach($reports as $r)
        <tr>
          ...
          <td class="user-name">
            <a class="name" href="{{ route('profile', ['id' => $r->reporter_id ]) }}">{{ $r->reportuser->username }}</a>
          </td>
          <td class="user-name">
            <a class="name" href="{{ $r->staff_id ? route('profile', ['id' => $r->staff_id ]) : route('home')}}">{{ $r->staff_id ? $r->staffuser->username : "" }}</a>
          </td>
          <td>
						@if($r->solved == 0)
