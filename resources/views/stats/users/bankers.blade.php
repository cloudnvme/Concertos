        ...
        @foreach($bankers as $key => $b)
          <tr>
            ...
            <td @if(auth()->user()->username == $b->username) class="mentions" @endif>
              ...
              <span class="badge-user text-bold"><span class="text-orange"><i class="fa fa-eye-slash" aria-hidden="true"></i>{{ strtoupper(trans('common.hidden')) }}</span>@if(auth()->user()->id == $b->id || auth()->user()->group->is_modo)<a href="{{ route('profile', ['id' => $b->id]) }}">({{ $b->username }}</a></span>
              @endif
              @else
