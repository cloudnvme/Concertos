        ...
        <tbody>
          ...
          <tr>
            ...
            <td @if(auth()->user()->username == $d->username) class="mentions" @endif>
              ...
              <span class="badge-user text-bold"><span class="text-orange"><i class="fa fa-eye-slash" aria-hidden="true"></i>{{ strtoupper(trans('common.hidden')) }}</span>@if(auth()->user()->id == $d->id || auth()->user()->group->is_modo)<a href="{{ route('profile', ['id' => $d->id]) }}">({{ $d->username }}</a></span>
              @endif
              @else
