...
<div class="container">
<div class="block">
  <h2><a class="view-user" data-id="{{ $user->id }}" data-slug="{{ $user->username }}" href="{{ route('profile', ['id' => $user->id]) }}">{{ $user->username }}</a>
Warnings Log</h2>
  <hr>
  <div class="row">
  <div class="col-sm-12">
  ...
  <table class="table table-condensed table-striped table-bordered table-hover">
    ...
    @foreach($warnings as $warning)
      <tr>
        ...
        <td>
			<a class="view-torrent" data-id="{{ $warning->staffuser->id }}" data-slug="{{ $warning->staffuser->username }}" href="{{ route('profile', ['id' => $warning->staffuser->id]) }}">{{ $warning->staffuser->username }}</a>
        </td>
        <td>
